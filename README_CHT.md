[English](https://github.com/winw1010/tataru-assistant/blob/main/README.md) | 繁體中文 | [简体中文](https://github.com/winw1010/tataru-assistant/blob/main/README_CHS.md)

# Tataru Assistant 是什麼?

**Tataru Assistant** 為 FFXIV 國際版的即時劇情字幕翻譯程式，主要功能如下

- AI翻譯對話文字和過場字幕

- 支援AI多輪對話，使翻譯結果更準確

- 擷取螢幕文字並進行翻譯

- 根據[**對照表**](https://github.com/winw1010/tataru-assistant-text)修正翻譯結果，例如將**タタル**修正為**塔塔露**

- 自訂翻譯結果

# Tataru Assistant 的翻譯方式

## AI 翻譯

翻譯能力較佳，可很好地翻譯遊戲中各種較為口語的句子，推薦使用免費額度較高的Germini

- Gemini (推薦)

- ChatGPT

- Claude

- Cohere

- Kimi

- Ollama (本地LLM)

- 自訂LLM (僅適用相容於OpenAI的API)

## 傳統翻譯機 (不推薦使用)

可無限使用，但翻譯品質較差，不推薦使用

- 有道翻譯

- 百度翻譯

- 彩雲小譯

- Papago Naver

- DeepL

# 檔案下載

- [.NET 10 Desktop Runtime](https://dotnet.microsoft.com/zh-tw/download/dotnet/thank-you/runtime-desktop-10.0.11-windows-x64-installer) (Tataru Assistant 3.0 以上版本需安裝此Runtime才能運作)

- [Tataru Assistant 安裝檔](https://github.com/winw1010/tataru-assistant/releases/latest/download/Tataru_Assistant_Setup.exe)

# 檔案下載(舊)

- [.NET Framework 4.8](https://dotnet.microsoft.com/zh-tw/download/dotnet-framework/thank-you/net48-web-installer) (Tataru Assistant 2.x 版本需安裝此Runtime才能運作)

# 安裝步驟

1. 下載「Tataru Assistant 安裝檔」和「.NET 10 Desktop Runtime」

2. 執行「windowsdesktop-runtime-10.0.11-win-x64.exe」安裝 .NET 10 Desktop Runtime (Tataru Assistant 3.0 以上版本需安裝此Runtime才能運作)

3. 執行「Tataru Assistant Setup.exe」安裝 Tataru Assistant ，若顯示「Windows 已保護您的電腦」的訊息，請點選「其他資訊」，再點選下方的「仍要執行」

4. 點選視窗上的齒輪圖示開啟 Tataru Assistant 的設定視窗，切換到【翻譯設定】設置你的遊戲語言和翻譯語言，設定完畢後按儲存即可使用

5. 若安裝後無法自動翻譯，請檢查是否已關閉NVIDIA ShadowPlay，此功能會讓本程式無法顯示翻譯

6. 如果上述步驟做完仍無法翻譯，請至【設定】>【系統設定】，點選【修復字幕讀取器】，修復後重新開機即可

# 原始碼

## 建置原始碼

- 必要元件：[Git](https://git-scm.com)、[Node.js(長期維護版)](https://nodejs.org/zh-tw/)
- 編輯器：[Visual Studio Code](https://code.visualstudio.com/)

```bash
# 複製專案
git clone https://github.com/winw1010/tataru-assistant

# 進入專案資料夾
cd tataru-assistant

# 安裝套件
npm install

# 啟動APP
npm start
```

## 建立安裝檔

- 輸出位置：tataru-assistant/build
- 安裝檔設定位於 package.json 裡的 build 物件中，打包工具為 electron-builder

```bash
# 進入專案資料夾
cd (你的專案位置)

# 建立安裝檔
npm run dist
```

# Credits

- [Sharlayan](https://github.com/FFXIVAPP/sharlayan) - 用於讀取記憶體中的遊戲字幕
- [Electron](https://www.electronjs.org/) - APP框架

# Support

[<img src="https://github.com/winw1010/tataru-assistant/blob/main/src/html/img/bmc/bmc-button.png" alt="Buy me a coffee" width="200"/>](https://www.buymeacoffee.com/winw1010)
